...
export function MiniCalendarForYear({ year }: Props) {
  const yearDaysCountriesByMonth = useDataStore((store) => store.daysByCountryByMonthByYear[year]);
  ...
  const activeDays = useMemo(() => {
    ...
    return Object.values(yearDaysCountriesByMonth).reduce((stack: DateMMDD[], month) => {
      if (!month) {
        return stack;
      }

      const monthActiveDays = Object.entries(month).reduce((stack: DateMMDD[], [countryCode, activeDays]) => {
