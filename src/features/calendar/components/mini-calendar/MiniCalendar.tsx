...
export const MiniCalendar = ({ className, activeDays = [], children, daysInYear = 366 }: PropsWithChildren<Props>) => {
  ...
  return (
    <div
      className={cn(
        ...
        'pointer-events-none',
        'transition-bounce',
        'hidden [@media(min-width:860px)]:block',
        className,
      )}
    >
      {children && <strong className="block mb-3 text-sm tracking-wider font-semibold">{children}</strong>}
      ...
      <div className="grid grid-cols-4 gap-4">
        {DAYS_GROUPED_BY_MONTHS_BY_DAYS_IN_YEAR[daysInYear].map((month) => (
          <div key={month.monthNumber} className={cn('p-1.5 bg-[#3d3d3d6e] rounded-sm')}>
            <strong className="block mb-2 text-[10px] tracking-wider font-semibold">{t(month.name)}</strong>
            <div className={cn('grid grid-cols-7 gap-x-1 gap-y-1')}>
              {month.days.map((day) => (
                <span
                  key={day}
                  className={cn('inline-flex size-2 rounded-xs', 'bg-[#272620]', {
